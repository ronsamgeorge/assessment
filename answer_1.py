...
def server_cost(d1, m1, y1, d2, m2, y2):
    ...
    if (d2>= d1) and (m2 >= m1) and (y2>y1):    #if it is greater than a year 
        ...
        cost = 20 
    elif(d2>= d1)  and ( (m2 > m1) or (y2 > y1)): #if greater than a month y2 >y1 check because it can also get carried to different year in which case m2 will be less than m1
        if y2>y1 :
            m2 += 12
        cost = 1000 * (m2-m1)
